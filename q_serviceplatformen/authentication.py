...
def _get_saml_token(cvr: str, cert_path: str, entity_id: str, test: bool) -> Token:
    ...
    response = requests.post(url, json=payload, cert=cert_path, timeout=REQUEST_TIMEOUT_SECONDS)
...
def _get_access_token(saml_token: str, cert_path: str, test: bool) -> Token:
    ...
    response = requests.post(url, data=payload, cert=cert_path, timeout=REQUEST_TIMEOUT_SECONDS)
    response.raise_for_status()
    response = response.json()
